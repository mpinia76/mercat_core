...
use Mercat\Core\dao\IProductoDAO;
...
class ProductoDoctrineDAO extends CrudDAO implements IProductoDAO{
	...
	protected function enhanceQueryBuild(QueryBuilder $queryBuilder, ICriteria $criteria){
		...
		if( !empty($codigo) ){
			$queryBuilder->andWhere("upper(prod.codigo)  like :codigo");
			$queryBuilder->setParameter( "codigo" , "%$codigo%" );
		}
		
		$codigoExacto = $criteria->getCodigoExacto();
		if( !empty($codigoExacto) ){
			$queryBuilder->andWhere("prod.codigo= '$codigoExacto'");
		...
		$porcentajeGanancia2 = $criteria->getPorcentajeGanancia2();
		if( !empty($porcentajeGanancia2) ){
			$queryBuilder->andWhere("prod.porcentajeGanancia2 > 0");
