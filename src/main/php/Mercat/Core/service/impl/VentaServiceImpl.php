...
class VentaServiceImpl extends CrudService implements IVentaService {
	...
	/**
	 * (non-PHPdoc)
	 * @see src/main/php/Mercat/Core/service/Mercat\Core\service.IVentaService::cobrar()
	 */
	public function cobrar(Venta $venta, Cuenta $cuenta, User $user, $montoPagar, $montoActualizado){
		...
		if ($vendedor->getComision()) {
			...
			$comision = round($montoPagar*($vendedor->getComision()/100),1);
			$movimiento = new MovimientoVenta();
			$movimiento->setDebe($comision);
			$movimiento->setFecha( new \Datetime() );
			$movimiento->setHaber( 0 );
			$movimiento->setObservaciones("");
			$movimiento->setVenta($venta);
			$movimiento->setCuenta($cuentaComision);

			$movimiento->setConcepto( MercatUtils::getConceptoMovimientoComisionVenta() );
	...
	public function anular(Venta $venta, User $user){
		...
		if( $venta->getMonto() > $venta->getMontoDebe() ){
			...
			foreach ($movimientos as $movimiento) {
				...
				//Logger::log('Comparar conceptos: '.$movimiento->getConcepto()->getOid().'=='.MercatUtils::MERCAT_CONCEPTO_MOVIMIENTO_VENTA);
				$concepto = ($movimiento->getConcepto()->getOid()==MercatUtils::MERCAT_CONCEPTO_MOVIMIENTO_VENTA)?MercatUtils::getConceptoMovimientoAnulacionVenta() :MercatUtils::getConceptoMovimientoAnulacionComisionVenta();
...
/**
	 * (non-PHPdoc)
	 * @see src/main/php/Mercat/Core/service/Mercat\Core\service.IVentaService::devolver()
	 */
	public function devolver(Venta $venta, Cuenta $cuenta, User $user){
		...
		if( $venta->getMontoDevolucion() > $venta->getMontoDebe() ){
			...
			$movimiento->setConcepto( MercatUtils::getConceptoMovimientoDevolucion() );
			$movimiento->setUser($user);
	...
	/**
	 * (non-PHPdoc)
	 * @see src/main/php/Mercat/Core/service/Mercat\Core\service.IVentaService::cobrarCtaCte()
	 */
	...
	/**
	 * (non-PHPdoc)
	 * @see src/main/php/Mercat/Core/service/Mercat\Core\service.IVentaService::getTotalesDia()
	 */
	...
	/**
	 * (non-PHPdoc)
	 * @see src/main/php/Mercat/Core/service/Mercat\Core\service.IVentaService::getTotalesMes()
	 */
     ...
     * @see src/main/php/Mercat/Core/service/Mercat\Core\service.IVentaService::getTotalesMes()
     */
    public function getRankProductos( $criteria, $productos){

        try{

            $dao = $this->getDAO();;

            $result = $dao->getRankProductos($criteria, $productos);
    ...
    public function getRankClientes( $criteria){

        try{

            $dao = $this->getDAO();;

            $result = $dao->getRankClientes($criteria);
