<?php
namespace Mercat\Core\notificaciones\backupBBDD;

use Mercat\Core\utils\XTemplate;
...
class BackupBBDD{
	...
	function backupDatabaseTables($dbHost,$dbUsername,$dbPassword,$dbName, $filename,$tables = '*' ){
	    ...
	    //save file
		$nameFile ='db_mercat_' . date("Y-m-d-H-i-s") .'.sql';
	...
	public function send(){
		...
		$this->setPathBackup(MercatConfig::getInstance()->getAppPathCore()."/backup/");
		
		$filename = 'db_mercat_' . date("Y-m-d-H-i-s") . '.zip';
		$backupFile = $this->getPathBackup().$filename;

		self::backupDatabaseTables(MercatConfig::getInstance()->getDbHost(),MercatConfig::getInstance()->getDbUser(),MercatConfig::getInstance()->getDbPassword(),MercatConfig::getInstance()->getDbName(),$filename);
		...
		$subjectMail = 'BackUp cose_mercat';
		...
		$template->assign("titulo", "En el archivo adjunto se encuentra el BackUp de la BBDD cose_mercat realizado el ".date('d/m/Y H:i:s'));
		...
		MercatUtils::sendMail(MercatConfig::TEST_MAIL_TO,MercatConfig::TEST_MAIL_TO, $subjectMail, $mensaje, $attachs);
		unlink($backupFile);
